...
async function calculate({ shape, dimentions, stone, chamfer, additional }) {
  if (
    shape === "multicorner" &&
    +dimentions.length_c < +dimentions.length_a * 2 + 500
  ) {
    return { message: 'Значение размера "С" слишком мало.' };
  }
  let lengthsSum = 0;
  let lengthToCut = 0;
  ...
  const correction =
    (Object.values(dimentions).filter((value) => value !== "").length - 2) * 2;
  ...
  const sum =
    ...
    (additional.sink ? 1 : 0) *
      (processPrice.cuttingWashing + processPrice.drillWaterTap) +
    (((additional.montage ? 1 : 0) * lengthsSum) / 1000) *
      processPrice.mountCountertop +
...
async function getLengthSlab(stone) {
  const data = await connection
    .promise()
    .query("SELECT * FROM " + `${stone.producer}` + " WHERE " + "color=?", [
      stone.color,
    ]);
  const { normalLength, jamboLength, normalPrice, jamboPrice } = data[0][0];
  if (normalLength) {
    return {
      price: normalPrice,
      length: normalLength,
...
async function getAllPrice(chamfer) {
  const chamferPriceArray = await connection
    .promise()
    .query("SELECT price FROM chamfers WHERE type=?", [chamfer]);
  const chamferPrice = chamferPriceArray[0][0].price;
  const processPriceArray = await connection
    .promise()
    .query("SELECT * FROM processingPrice");
